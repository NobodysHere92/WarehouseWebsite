...
function createHeader(active) {
  ...
  return `
    <div class="nav-container">
      ...
      <a class="cta-btn nav-cta" href="${basePath}request-quote.html">Request a Quote</a>
    </div>
  `;
}
...
function setupServiceAccordions() {
  const cards = Array.from(document.querySelectorAll('details.service-card'));
  ...
  syncState(mobileQuery.matches);
  mobileQuery.addEventListener('change', event => syncState(event.matches));
}
